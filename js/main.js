 ...
 */
{
	...
	const settings = {
		...
		details: {duration: 300, delay: 0, easing: [0.8,0,0.2,1]},
		...
		detailsCtrl: {duration: 300, easing: 'linear'},
		...
		detailsItems: {duration: 800, easing: [0.8,0,0.2,1]},
	...
	class Entry {
        ...
        init() {
			...
			// details
			this.DOM.details = {
				wrapper: this.DOM.el.querySelector('.section__details'),
				items: Array.from(this.DOM.el.querySelectorAll('.section__details > .section__details-item'))
			};
		}
		show(direction) {
			this.isHidden = false;
		...
		toggle(direction) {
			...
			return Promise.all([this.toggleTitle(!this.isHidden),
								...
								this.toggledetails(!this.isHidden)]);
		}
		toggleTitle() {
		...
		toggledetails() {
			anime.remove(this.DOM.details.items);
			return anime({
				targets: this.DOM.details.items.slice(0, 2),
				duration: settings.details.duration,
				delay: (target, index) => {
					return !this.isHidden ? index * 40 + settings.details.duration * 0.5 + settings.details.delay : index * 40 + settings.details.delay;
				},
				easing: settings.details.easing,
				translateY: this.isHidden ? [this.DOM.details.ty, this.direction === 'next' ? this.DOM.details.ty-20 : this.DOM.details.ty+20] : [this.direction === 'next' ? this.DOM.details.ty+20 : this.DOM.details.ty-20, this.DOM.details.ty],
				opacity: this.isHidden ? 0 : 1
			}).finished;
		}
    };
    ...
    class Slideshow {
        ...
        init() {
			...
			this.DOM.detailsContainer = this.DOM.el.querySelector('.details');
			this.DOM.detailsCtrls = {
				toggle: this.DOM.detailsContainer.querySelector('.details__toggle'),
				more: this.DOM.detailsContainer.querySelector('.details__toggle > .details__toggle-inner--more'),
				less: this.DOM.detailsContainer.querySelector('.details__toggle > .details__toggle-inner--less'),
			};
			this.DOM.previewCloseCtrl = this.DOM.detailsContainer.querySelector('.button-contentclose');
		...
		layout() {
			...
			const factEl = this.currentEntry.DOM.details.items[0];
			const factHeight = factEl.getBoundingClientRect().height + parseFloat(window.getComputedStyle(factEl).marginBottom);
			const paddingdetailsStyle = window.getComputedStyle(this.currentEntry.DOM.details.wrapper);
			const paddingdetails = parseFloat(paddingdetailsStyle.paddingTop) + parseFloat(paddingdetailsStyle.paddingBottom);

			this.detailsTranslation = win.height - 2 * factHeight - paddingdetails;
			for ( let i = 0; i <= this.entriesTotal - 1; ++i ) {
				const entry = this.DOM.entries[i];
				entry.DOM.expander.style.transform = `scale3d(0.54,1,1) translate3d(0px,${this.detailsTranslation}px,0px)`;
				for ( let j = 0, len = entry.DOM.details.items.length; j <= len - 1; ++j ) {
					entry.DOM.details.ty = this.detailsTranslation;
					const item = entry.DOM.details.items[j];
					item.style.transform = `translate3d(0px,${this.detailsTranslation}px,0px)`;
					if ( j > 1 ) {
		...
		initEvents() {
			...
			// details Container
			this.DOM.detailsCtrls.toggle.addEventListener('click', () => this.toggledetailsContainer());

			// details (clickable details)
			for ( let i = 0; i <= this.entriesTotal - 1; ++i ) {
				const entry = this.DOM.entries[i];
				entry.DOM.details.items
					 .filter(fact => fact.classList.contains('section__details-item--clickable'))
			...
			this.onResize = () => {
				...
				if ( this.isdetailsOpen ) {
					// Toggle the detailsCtrls state
					this.DOM.detailsCtrls.more.style.opacity = 1;
					this.DOM.detailsCtrls.less.style.opacity = 0;
					this.isdetailsOpen = !this.isdetailsOpen;
					this.toggleNavigationCtrls({opacity: 1, duration: 1});
					this.isdetailsAnimating = false;
				}
				if ( this.gallery ) {
					this.DOM.previewCloseCtrl.style.opacity = 0;
					this.toggleGallery(this.gallery, {duration: 1,opacity: 0}).then(() => this.gallery = null);
				}
				this.DOM.el.classList.remove('sections--detailsopen');
			};
			window.addEventListener('resize', debounce(() => this.onResize(), 20));
		}
		navigate(direction) {
			if ( this.isEntriesAnimating || this.isdetailsAnimating ) return;
		...
		update(newEntry) {
			...
			if ( this.isdetailsOpen ) {
				this.toggledetailsContainer().then(updateFn);
			}
			else {
				updateFn();
			}
		...
		toggledetailsContainer() {
			...
			this.isdetailsAnimating = true;
			return Promise.all([this.toggledetailsCtrl(), this.animateExpander(), this.animatedetailsItems()]).then(() => {
				this.isdetailsOpen = !this.isdetailsOpen;
				this.isdetailsAnimating = false;
			});
		}
		preview(gallery) {
			if ( this.isdetailsAnimating || !gallery ) return;
			this.isdetailsAnimating = true;

			this.gallery = gallery;
			this.DOM.el.classList.add('sections--detailsopen');
			Promise.all([
				this.toggleNavigationCtrls({
					opacity: 0
				}),
				this.toggledetailsCtrl({
					delay: 0,
					opacity: 0
				}),
			...
			]).then(() => this.isdetailsAnimating = false);
		}
		closePreview() {
			if ( this.isdetailsAnimating ) return;
			this.isdetailsAnimating = true;
		...
		animateExpander(animeconfig) {
			return this.animate(Object.assign({
				...
				delay: !this.isdetailsOpen ? 0 : 300,
				translateY: !this.isdetailsOpen ? [this.detailsTranslation, 0] : this.detailsTranslation,
				scaleX: [0.54,0.54]
			}, animeconfig));
		}
		animatedetailsItems(animeconfig) {
			return this.animate(Object.assign({
				targets: this.currentEntry.DOM.details.items,
				duration: settings.detailsItems.duration,
				easing: settings.detailsItems.easing,
				delay: (target, index, total) => !this.isdetailsOpen ? (index+1) * 30 + 150 : (total-index-1) * 30,
				translateY: !this.isdetailsOpen ? [this.detailsTranslation,0] : this.detailsTranslation,
				opacity: (target, index) => !this.isdetailsOpen ? 1 : index > 1 ? 0 : 1
			}, animeconfig));
		}
		toggledetailsCtrl(animeconfig) {
			return this.animate(Object.assign({
				targets: [this.DOM.detailsCtrls.more, this.DOM.detailsCtrls.less],
				duration: settings.detailsCtrl.duration,
				easing: settings.detailsCtrl.easing,
				opacity: (target, index) => index ? !this.isdetailsOpen ? 1 : 0 : !this.isdetailsOpen ? 0 : 1
			}, animeconfig));
		}
